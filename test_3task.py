...
def parse_args():
    p = argparse.ArgumentParser(description="RCR-IR three-task evaluation")
    p.add_argument("--ckpt_path", type=str, required=True)
    p.add_argument("--data_file_dir", type=str, default="")
    p.add_argument("--device", type=str, default="cuda:0")
    p.add_argument("--state", choices=["auto", "ema", "raw"], default="auto")
    p.add_argument("--output_dir", type=str, default="results_rcr_ir/3task")
...
def main():
    ...
    # PromptIR-style three-task table: CBSD68 x3 + Rain100L + SOTS.
    benchmarks = ["denoise_15", "denoise_25", "denoise_50", "derain", "dehaze"]
    results = {}

    # Keep one global NumPy RNG stream across the three denoising levels,
    # matching the reference evaluation sequence.
